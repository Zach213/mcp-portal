...
function registerTools(server, z, sessionState, getSessionId) {
  ...
  server.tool(
    ...
    [
      ...
      '',
      'NODE C — Draft review then immediate deploy (1 credit):',
      '  Show the user what you\'re about to deploy:',
      '    Watch: scenes + greeting + example Q&A.',
      '    Play: blocked selectors + greeting + allowed URLs + knowledge.',
      '  Then call make_portal immediately. Do NOT ask "ready to deploy?" or wait for confirmation.',
      '  Portal is an artifact — create first, refine after. Auto-open the portal URL when ready.',
      '',
      'NODE D — Post-deploy (always offer these):',
      '  D1: "Want to name it?" → configure_portal.',
      '  D2: "Want to embed it?" → configure_embed (ask for origin, return iframe + console preview).',
      '  D3: "Want to limit uses?" → configure_portal with max_uses.',
      '  D4: "Want session replays?" → get_portal_sessions.',
      '  D5: "Not happy? I can recreate it." → back to Node B.',
      '  Show the portal config summary inline so the user can request edits.',
      '',
  ...
  server.tool(
    ...
    [
      ...
      '',
      'For watch mode: prefer create_script for scene quality. Show the draft to the user then deploy immediately — do NOT ask for confirmation.',
      'After deploying, offer: naming (configure_portal), embedding (configure_embed), or iteration.',
      '',
  ...
  server.tool(
    ...
    [
      ...
      'Show the user the compiled scenes/greeting/Q&A, then call make_portal immediately.',
      'Do NOT ask "ready to deploy?" — just show the draft and deploy.',
      'After deploying, offer: naming (configure_portal), embedding (configure_embed), or iteration.',
    ].join('\n'),
    { session_id: z.string() },
    async ({ session_id }) => {
  ...
  server.tool(
    ...
    [
      ...
      'then call make_portal immediately. Do NOT ask "ready to deploy?".',
      'After deploying, offer naming/embedding/iteration.',
    ].join('\n'),
    {
  ...
  server.tool(
    ...
    [
      ...
      'Show the user the selectors, then call make_portal immediately. Do NOT ask "ready to deploy?".',
      'After deploying, offer naming/embedding/iteration.',
    ].join('\n'),
    { session_id: z.string() },
    async ({ session_id }) => {
  ...
  server.tool(
    ...
    [
      ...
      'When status is "draft", show the user the scenes/greeting/Q&A, then call make_portal immediately.',
      'Do NOT ask "ready to deploy?" — show the draft and deploy. After deploying, offer naming/embedding/iteration.',
    ].join('\n'),
    {
