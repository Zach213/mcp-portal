...
function registerTools(server, z, sessionState, getSessionId) {
  ...
  server.tool(
    ...
    [
      ...
      '',
      'Schema: entry.url (required), experience.mode ("play"|"watch"), experience.agent.goal (required for watch),',
      '  agent.greeting, agent.scenes [{script, actions [{action, selector?, text?, ms?}]}],',
      '  guardrails.allowed_urls, guardrails.disabled_elements.',
